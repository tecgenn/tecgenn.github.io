...
const Testimonials: React.FC = () => {
  return (
    <section id="testimonials" className="py-12 sm:py-16 relative">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 bg-[#0A0A1A] rounded-2xl py-16 sm:py-20 shadow-depth">
        <SectionHeader
          title="O Que Nossos Clientes Dizem"
          description="Resultados reais de pessoas que transformaram seus negócios com a TecGenn."
        />

        <div className="mt-16 grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {testimonials.map((testimonial, index) => (
            <Card key={index} className="justify-between" glow={false}>
              <div>
                <div className="flex">
                  {[...Array(5)].map((_, i) => (

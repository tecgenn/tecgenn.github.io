...
// FIX: Explicitly type StarIcon as a React.FC to allow React-specific props like `key` and prevent type errors in loops.
...
const Testimonials: React.FC = () => {
  return (
    <section id="testimonials" className="py-20 sm:py-28 bg-[#0A0A1A]/50 bg-grid-cyan-500/[0.05]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center">
          <h2 className="text-3xl sm:text-4xl font-extrabold text-white">O Que Nossos Clientes Dizem</h2>
          <p className="mt-4 max-w-2xl mx-auto text-lg text-gray-400">Resultados reais de pessoas que transformaram seus negócios com a TecGenn.</p>
        </div>

        <div className="mt-16 grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {testimonials.map((testimonial, index) => (
            <div key={index} className="bg-gray-900/50 border border-cyan-500/20 rounded-xl p-8 flex flex-col justify-between transform transition-transform duration-300 hover:scale-105 hover:border-cyan-500/50">
              <div>
                <div className="flex">
